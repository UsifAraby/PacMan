...
MainMenu::MainMenu(float width, float length) {
	...
	/*Map mp;
	int tmp = mp.scoreStack.top();
	if (tmp > getHighestScore())
	{
		newhighscore = tmp;
	}*/

	string loseOptions[] = { "Your Highst Score: " + to_string(getHighestScore()), "Replay The Current Game", "Return To Main Menu" };
	for (int i = 0; i < 3; i++) {
		Text text;
		text.setFont(font);
